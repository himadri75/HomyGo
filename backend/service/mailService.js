...
const sendAccountCreatedMail = async (name, email, gender, dob, req) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"Homygo Corporation" <${process.env.SMTP_MAIL}>`,
      to: email,
...
const sendLoginSuccessMail = async (user, req) => {
  try {
    // Extract IP safely (works behind proxies too)
    const ip =
      req.headers["x-forwarded-for"]?.split(",")[0] ||
      req.socket?.remoteAddress ||
      req.ip;
...
// Emergency SOS Mail
const sendEmergencySOSMail = async (emergencyEmail, user, location, req) => {
  try {
    const ip =
      req.headers["x-forwarded-for"]?.split(",")[0] ||
      req.socket?.remoteAddress ||
    ...
    const mailOptions = {
      from: `"Homygo Emergency SOS" <${process.env.SMTP_MAIL}>`,
      to: emergencyEmail,
